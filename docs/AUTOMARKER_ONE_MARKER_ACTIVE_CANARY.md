...
No separate leader field is required. The game rejects marker placement by a non-leader, so the bridge treats only a fresh, authenticated, exact outbound `World.UseSlot` method-249858 carrier from the locally process/SYN/tuple-bound game connection as game-authority evidence. It never synthesizes a request without that carrier. The existing coordinator continuity boolean is set internally from this evidence; it is not accepted from command-line input or invented by a packet-field decoder. Method 46 is exclusively the authoritative inbound `SyncToMeDeltaInfo` marker-add confirmation route.
