...
only the packages actually discovered on disk. Website pairing validates a
one-time app token with the submission service before the native host stores it
in Windows Credential Manager. Endpoint settings are synced and atomically
replaced, and a failed endpoint write restores the prior credential so account
updates cannot silently disconnect a working installation. Tokens never enter
browser storage, settings JSON, or plug-in folders.
