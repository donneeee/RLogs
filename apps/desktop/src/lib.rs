...
mod automarker_active_coordinator;
#[cfg(windows)]
...
#[cfg(windows)]
#[allow(dead_code)]
#[path = "../../../plugins/games/blue-protocol-star-resonance/src/automarker_windivert_backend.rs"]
...
fn compact_live_overlay_run_projection(
...
fn enrich_bpsr_live_ability_presentation(
        ...
        .map(|snapshot| {
            ...
            LiveOverlayEncounterPresentation {
                ...
                run_projection: update.run_projection.as_ref().map(|run| {
                    compact_live_overlay_run_projection(
                        run,
                        &snapshot.deployment_id,
                        &snapshot.client_build,
                        &snapshot.protocol_pack_digest,
...
struct LiveCombatFeedState {
    ...
    /// Latest exact scene observation, regardless of whether it came from a
    /// canonical packet, the exact dungeon reducer, or the reviewed native
...
struct RuntimeController {
    ...
    live_automarker_native_bridge: Arc<AutomarkerNativeBridgeLifecycle>,
...
impl RuntimeController {
    ...
    fn new_with_developer_tools_and_application_version(
        ...
        Ok(Self {
            ...
            live_automarker_native_bridge: Arc::new(AutomarkerNativeBridgeLifecycle::default()),
    ...
    fn start_live(self: &Arc<Self>, request: LiveSessionRequest) -> Result<(), String> {
        ...
        let native_scene_observer = if target.build_id == native_scene_observer::REVIEWED_BUILD {
            ...
            let automarker_native_bridge = Arc::clone(&self.live_automarker_native_bridge);
            ...
            let observer = native_scene_observer::NativeSceneObserver::spawn(
                ...
                move |update| {
                    ...
                    automarker_native_bridge.reconcile_context(current_automarker_context.as_ref());
        ...
        let automarker_bridge_session = AutomarkerBridgeSessionIdentity {
        ...
        self.live_automarker_native_bridge.accept_parser_evidence(
            initial_automarker_context.as_ref(),
            self.live_automarker_bridge_evidence.current(),
        );
        ...
        let supervisor_live_automarker_native_bridge =
            Arc::clone(&self.live_automarker_native_bridge);
            ...
            .spawn(move || {
                ...
                let capture_result = (|| -> Result<_, String> {
                    ...
                    loop {
                        ...
                        let mut live_scene_changed = false;
                            ...
                            || (live_dirty && last_live_publish.elapsed() >= live_refresh_interval)
                        {
                            ...
                            if reviewed_dungeon {
                                match live_encounter.live_snapshot() {
                                    Ok(run_snapshot) => {
                                        if live_dungeon_active
                                            && let Some(scene_id) = run_snapshot
                                                .runs
                                                .last()
                                                ...
                                                .allows(scene_id, map_id)
                                        {
                                            ...
                                            // identity for live presentation when a world
                                            // transition packet was missed; do not manufacture
                                            // a canonical packet event.
                ...
                live_automarker_native_bridge.finish_session(&session_id);
            ...
            Ok(worker) => {
                thread::Builder::new()
                    ...
                    .spawn(move || {
                        ...
                        supervisor_live_automarker_native_bridge
                            .finish_session(&supervisor_session_id);
...
fn enrich_bpsr_live_rdps_effect_presentations(
    ...
    let effect_ids = snapshot
        ...
        .filter_map(|influence| influence.effect_id.parse::<i64>().ok())
        .collect::<BTreeSet<_>>();
    snapshot.rdps_effect_presentations = bpsr_rdps_effect_presentations(
        effect_ids,
        &snapshot.deployment_id,
        &snapshot.client_build,
        &snapshot.protocol_pack_digest,
...
fn live_overlay_event_requires_immediate_publish(event: &CanonicalEvent) -> bool {
    matches!(event, CanonicalEvent::WorldChanged(_))
}
...
mod tests {
    ...
    fn passive_automarker_start_key_precedes_scene_and_requires_exact_session_dependencies() {
        ...
        std::fs::write(root.join("WinDivert64.sys"), b"test").unwrap();
        let session = AutomarkerBridgeSessionIdentity {
    ...
    fn live_overlay_republishes_capture_time_identity_before_run_completion() {
        assert!(!live_overlay_topic_invalidates(EventTopic::World));
        ...
        assert!(live_overlay_event_requires_immediate_publish(
    ...
    fn packet_scene_presentation_supersedes_native_startup_fallback() {
        ...
        assert_eq!(
            feed.current(),
            None,
            "an unavailable native observation must not retain the prior Mech Facility context"
        );
        feed.set_native_scene_presentation(
            true,
            Some(native_scene_observer::NativeSceneIdentity {
        ...
        assert_eq!(
            feed.current().unwrap().scene_id,
            6_561,
            "fresh packet world evidence must replace the native startup fallback"
        );
        ...
        assert_eq!(
            feed.current(),
            None,
            "an unsupported packet scene must clear rather than expose a stale native family"
        );
        feed.set_native_scene_presentation(
            true,
            Some(native_scene_observer::NativeSceneIdentity {
    ...
    #[cfg(windows)]
    #[test]
    fn live_combat_packet_scene_supersedes_native_startup_fallback() {
        let feed = LiveCombatFeed::default();
        let mut packet_snapshot = CombatTimelineSnapshot {
            schema_version: 1,
            session_id: "live".into(),
            deployment_id: "global".into(),
            region_id: "global".into(),
    ...
    fn history_labels_cross_builds_while_action_semantics_stay_exact() {
        ...
        assert_eq!(
            current.runs[0].presentation_scene_name.as_deref(),
            Some("Chaotic - Tina's Mindrealm")
        );
        assert_eq!(
            current.runs[0].views[0].actors[0].abilities[0]
                .presentation_name
                .as_deref(),
            Some("Powerdraw")
        );
    ...
    fn live_overlay_ability_rows_share_exact_build_action_presentation() {
        ...
        let mut other_build = serde_json::json!({
            "ability_id": "2233",
